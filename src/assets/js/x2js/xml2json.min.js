				...
				function() {
					return function(z) {
						...
						function n(D, B, C) {
							...
							if (!(D[B] instanceof Array)
									&& z.arrayAccessFormPaths.length > 0) {
						...
						function a(G) {
							...
							if (E[6] && E[7]) {
								...
								C = 0 + (B == "-" ? -1 * C : C);
								F.setMinutes(F.getMinutes() - C
										- F.getTimezoneOffset());
							} else {
								if (G.indexOf("Z", G.length - 1) !== -1) {
									F = new Date(Date.UTC(F.getFullYear(), F
											.getMonth(), F.getDate(), F
											.getHours(), F.getMinutes(), F
											.getSeconds(), F.getMilliseconds()));
								}
							}
							return F;
						}
						...
						function A(D, J) {
							if (D.nodeType == h.DOCUMENT_NODE) {
								...
								if (D.nodeType == h.ELEMENT_NODE) {
										...
										if (K.__cnt == 1 && K.__text != null) {
											...
											if (K.__cnt == 1
												...
												if (K.__cnt > 1
													...
													if ((z.stripWhitespaces && K.__text == "")
															|| (K.__text.trim() == "")) {
														delete K.__text;
													}
												}
									...
									delete K.__cnt;
									if (z.enableToStringFunc
											&& (K.__text != null || K.__cdata != null)) {
										K.toString = function() {
									...
									if (D.nodeType == h.TEXT_NODE
											|| D.nodeType == h.CDATA_SECTION_NODE) {
										return D.nodeValue;
									}
								}
							}
						...
						function o(I, F, H, C) {
							var E = "<"
									+ ((I != null && I.__prefix != null) ? (I.__prefix + ":")
											: "") + F;
							if (H != null) {
								for (var G = 0; G < H.length; G++) {
									...
									E += " "
											+ D
													.substr(z.attributePrefix.length)
											+ "=";
						...
						function y(C, B) {
							if ((z.arrayAccessForm == "property" && v(B
									.toString(), ("_asArray")))
						...
						function l(D, B, C) {
							return z.jsonPropertiesFilter.length == 0
									|| C == ""
